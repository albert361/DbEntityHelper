...
namespace SMBCTPE.Helper
{
    ...
    public class DBHelper
    {
        ...
        public static Table GetWorkFlow(string PGName, string TypeName, string Status, string UGrpName, string UpDown)
        {
            Dictionary<String, object> parameters = new Dictionary<string, object>();
            parameters.Add("PGName", PGName);
            parameters.Add("TypeName", TypeName);
            parameters.Add("Status", Status);
            parameters.Add("UGrpName", UGrpName);
            parameters.Add("UpDown", UpDown);
            return DbAccess.Instance.ExecuteSPQuery("sp_WorkFlowHelper", parameters);
        }
        ...
        public static void Batch<T>(string sqlQuery, BatchObject<T> bo, Action<BatchObject<T>> action, int countsPerAction, int timesAction)
            where T : Entity
        {
            using (DbConnection con = SMBCTPE.DataAccess.DbAccess.Instance.GetConnection())
            {
                DbCommand cmd = null;
